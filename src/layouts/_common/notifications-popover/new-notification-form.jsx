...
export default function NewNotificationForm({ handleCloseDialog, fetchAllData }) {
  ...
  const FieldsSchema = Yup.object().shape({
    content: Yup.string().required('Mesaj Zorunlu'),
  });
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    const response = await WebServices.createNotification(sendData, isForm);
  ...
  const formSection = (
    <Stack spacing={2}>
      <RHFTextField name="content" label="Bildirim Giriniz" multiline rows={4} />
    </Stack>
  );
