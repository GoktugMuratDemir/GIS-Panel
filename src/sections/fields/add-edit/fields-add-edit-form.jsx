...
export default function FieldsAddEditForm() {
  ...
  async function fetchIdData() {
    const { data } = await WebServices.getFieldGetById({ Id: id });
  ...
  useEffect(() => {
    if (id) {
      ...
      setValue('coordinates', resByIdData?.coordinates || []);
    }
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    if (id) {
      const response = await WebServices.UpdateField(sendData, isForm);
      ...
      const response = await WebServices.createField(sendData, isForm);
  ...
  const formSection = (
    <Stack spacing={2}>
      <RHFTextField name="name" label="Tarla Adı" />
      <RHFTextField name="description" label="Açıklama" />

      {/* {!id ? <AddLocation /> : <EditLocation />} */}
      <EditLocationComponent
        setValue={setValue}
        valueTitle="coordinates"
        resByIdData={resByIdData}
      />
    </Stack>
  );
