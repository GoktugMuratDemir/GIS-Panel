...
const TABLE_HEAD = [
  { id: 'accountName', label: 'İşçi' },
  { id: 'jobTypeName', label: 'İş Tipi' },
  { id: 'fieldName', label: 'Tarla' },
  { id: 'date', label: 'Tarih' },
...
export default function WorkProgramList() {
  ...
  const [tableData, setTableData] = useState(resDataAll);
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      const response = await WebServices.DeleteProgram({ Id: id });
      if (response.success) {
        enqueueSnackbar('İş Programı Silindi');
              ...
              {dataFiltered
                ...
                .map((row) => (
                  <CustomTableRow
                    ...
                    row={row}
                    selected={selected.includes(row.assignmentId)}
                    onDeleteRow={() => handleDeleteRow(row.assignmentId)} // direkt api içindeki id yi alıyor
                  />
                ))}
...
function applyFilter({ inputData, comparator, filterName }) {
  ...
  if (filterName) {
    inputData = inputData.filter(
      (user) => user.accountName.toLowerCase().indexOf(filterName.toLowerCase()) !== -1
    );
  }
