...
const TABLE_HEAD = [
  ...
  { id: 'fieldName', label: 'Çalışılan Alan' },
  { id: 'description', label: 'Açıklama' },
  { id: 'workDay', label: 'İş Günü' },
  // { id: 'description', label: 'Açıklama' },
  { id: '', width: 88 },
];
...
export default function WorksList() {
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      const response = await WebServices.deleteSlider({ Id: id });
      if (response.success) {
        enqueueSnackbar('Slider Silindi');
...
function applyFilter({ inputData, comparator, filterName }) {
  ...
  if (filterName) {
    inputData = inputData.filter(
      (user) => user.worker.toLowerCase().indexOf(filterName.toLowerCase()) !== -1
    );
  }
