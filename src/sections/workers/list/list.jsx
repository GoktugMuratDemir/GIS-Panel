...
const TABLE_HEAD = [
  ...
  { id: 'isAdmin', label: 'Kullanıcı' },
  // { id: 'workField', label: 'Çalışma Alanı' },
...
export default function WorkerList() {
  ...
  // console.log('resDataAll workers : ', resDataAll);

  const [filterName, setFilterName] = useState('');
  const [tableData, setTableData] = useState(null);

  // console.log('tableData workers : ', resDataAll);
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      const response = await WebServices.DeleteWorker({ Id: id });
      if (response.success) {
        enqueueSnackbar('İşçi Silindi');
